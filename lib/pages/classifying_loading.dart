...
class ClassifyingLoading extends StatefulWidget {
  ...
  const ClassifyingLoading({Key? key, required this.videoFile}) : super(key: key);
...
class _ClassifyingLoadingState extends State<ClassifyingLoading> {
      ...
      classify().then((r){
        ...
        Future.delayed(delayDuration, () {
              Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                       ClassificationDone(prediction: _prediction,), // Navigate to the next page
                ),
              );
            });
      });
  ...
  Future classify() async{
     final videoFrames = await extractFrames(widget.videoFile, fps: 1);
      ...
      for(var i = 0; i < videoFrames.length; i++) {
        ...
        List<double> keypoints = keypointsWithScore.expand((list1) => list1.expand((list2) => list2.expand((list3) => list3))).toList();
        print(keypoints);
  ...
  Future<void> loadInterpreters() async {
    try {
      ...
      final moveNet = await Interpreter.fromAsset('assets/models/movenet_thunder.tflite', options: moveNetOptions); // Load the model from assets
      moveNet.allocateTensors();
      _moveNet =
        await IsolateInterpreter.create(address: moveNet.address);
      var classifierOptions = InterpreterOptions();
      classifierOptions.threads = 4;
      final classfier = await Interpreter.fromAsset('assets/models/classifier.tflite', options: classifierOptions); // Load the model from assets
      classfier.allocateTensors();
      _classifier =
        await IsolateInterpreter.create(address: classfier.address);
...
img.Image cropAndResize(img.Image image, Map<String, double> cropRegion, List<int> cropSize) {
  ...
  // Crop image
  img.Image outputImage = img.copyCrop(image, cropLeft, cropTop, cropRight - cropLeft, cropBottom - cropTop);

  // Add padding
  outputImage = addPadding(outputImage, paddingTop, paddingBottom, paddingLeft, paddingRight);
  // Resize image
  outputImage = img.copyResize(outputImage, width: cropSize[0], height: cropSize[1]);

  return outputImage;
}

img.Image addPadding(img.Image image, int top, int bottom, int left, int right) {
  img.Image paddedImage = img.Image(image.width + left + right, image.height + top + bottom);
  ...
  Future<List<List<List<List<double>>>>> moveNetInference(img.Image image, img.Image baseImage ,Map<String, double> cropRegion) async{
      ...
      for (int i = 0; i < 17; i++) {
        var y = baseImage.height *  (cropRegion['y_min']! + cropRegion['height']! * keypointsWithScores[0][0][i][0]);
        var x = baseImage.width * (cropRegion['x_min']! + cropRegion['width']! * keypointsWithScores[0][0][i][1]);
  ...
  dynamic classifierInference( List<List<double>> keypoints) async{
      ...
      int index =  output[0].asMap().entries.reduce((a, b) => a.value > b.value ? a : b).key;
      return {
        "index": index,
          ...
          children: [
            Positioned(
              ...
              child:Column(
                ...
                totalFrames !=0?  Text("$processedFrame/$totalFrames") : const Text("Extracting Frames")
              ],)
            ),
          ],
        ),
